...
class MiVentanaPrincipal(QMainWindow):
    ...
    def __init__(self):
        ...
        miLayoutEtiquetasInput.addWidget(miEtiquetaPrueba, 2, 0)
        ...
        # Agrandar txt
        miLayoutEtiquetasInput.addWidget(self.miInputPrueba, 2, 1, 1,3)

        miWidgetLayoutEtiquetasInput = QWidget()
        miWidgetLayoutEtiquetasInput.setLayout(miLayoutEtiquetasInput)

        # 1.2. Ciudad
        miEtiquetaFormato = QLabel("Ciudad")
        self.miComboFormato = QComboBox()
        self.miComboFormato.setFixedSize(700,27)

        miLayoutFormato = QHBoxLayout()
        miLayoutFormato.addWidget(miEtiquetaFormato)
        miLayoutFormato.addWidget(self.miComboFormato)
        datosCombo = ["Vigo","Nigran","Vilaza"]
        self.miComboFormato.addItems(datosCombo)
        ...
        miBotonBorrar.clicked.connect(self.boton_borrar_clicked)
        miBotonDefecto = QPushButton("Por defecto")
        miBotonDefecto.clicked.connect(self.boton_Por_defecto_clicked)
    ...
    def boton_engadir_clicked(self):
        ...
        tratamento = self.miInputTratamento.text()
        formato = self.radio_clicked()
        formato2 = self.miComboFormato.currentText()
        campos = (nome, apelidos, tlf, tratamento)
        self.miLista.addItem(f" Nome : {nome}, "
                             f"Apelidos : {apelidos}, "
                             f"Telefono :{tlf} , "
                             f"Tratamento : {tratamento} , "
                             f"Formato de correo : {formato} , "
                             f"Ciudad : {formato2}")
        self.miInputNome.setText("")
        self.miInputApelido.setText("")
        self.miInputTelefono.setText("")
        self.miInputTratamento.setText("")
    ...
    def boton_editar_clicked(self):
        ...
        self.miInputTratamento.setText(txtLista[3])


    def boton_borrar_clicked(self):
        itemSeleccionado = self.miLista.currentItem()
        txtItem = itemSeleccionado.text()
        print(f"Has seleccionado borrar la fila: {txtItem}")
        fila = self.miLista.currentRow()
        self.miLista.takeItem(fila)

    def boton_Por_defecto_clicked(self):
        self.miLista.clear()
        self.miInputNome.clear()
        self.miInputApelido.clear()
        self.miInputTelefono.clear()
        self.miInputTratamento.clear()
